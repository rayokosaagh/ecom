...
export function Card({
  ...
  return (
    <div
      className={cn(
        "rounded-xl transition-shadow duration-200 ease-standard",
        VARIANTS[variant],
        interactive &&
...
export function CardTitle({ className, ...props }: HTMLAttributes<HTMLHeadingElement>) {
  return (
    <h3
      className={cn("text-on-surface text-title-lg", className)}
      {...props}
    />
  );
}
...
export function CardDescription({
  ...
  return (
    <p className={cn("text-on-surface-variant text-body-md mt-1", className)} {...props} />
  );
}
