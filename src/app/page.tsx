 ...
 *
 * Handed to `FeaturedShowcase` as its `lead` rather than rendered here, so it
 * sits beside the featured product as one half of the hero. Every alignment is
 * stated rather than inherited: it is centred while the two columns are stacked
 * on a narrow window and left-aligned from `lg`, where the product is beside it
 * and copy centred against a card would read as an unrelated caption.
 */
function HeroCopy({
  categoryNames,
  showRegister,
  ...
  return (
    <div className="w-full text-center lg:text-left">
      <p className="animate-rise eyebrow text-primary flex items-center justify-center gap-2 lg:justify-start">
      ...
      {/* A step down from `display-lg`, because the copy is in a column: 72px
          in half a page breaks "Precision gear for" over three lines and leaves
          the headline taller than the product it is introducing. */}
      <h1 className="animate-rise rise-2 text-on-surface text-display-sm sm:text-display-md mx-auto mt-6 max-w-3xl lg:mx-0">
        Precision gear for
        <span className="accent-word block pt-2">the modern desk.</span>
      </h1>

      <div className="animate-rise rise-3 mt-9 flex flex-wrap items-center justify-center gap-3 lg:justify-start">
        <Link
          href="/products"
      ...
      {/* `mt-10` rather than the `mt-12` this had across a full page: beside
          the product the column has a card to keep level with rather than a
          whole page to breathe into. */}
      <ul className="animate-rise rise-4 text-on-surface-variant text-body-md mt-10 flex flex-wrap items-center justify-center gap-x-8 gap-y-2 lg:justify-start">
...
export default async function HomePage() {
  ...
  // The featured list, with what shoppers have said about each of them.
  ...
  return (
    <div className="bg-surface-container-low flex min-h-dvh flex-col">
      ...
      <main>
        {/* The front door: one section rather than two.

            The opening line, the actions and the featured spotlight used to be
            stacked — a centred hero, and then the same shop's best product
            introduced again a screen and a half further down. Both were saying
            "start here", which meant the page said it twice and neither one got
            the whole of the fold. Side by side they say it once: the copy makes
            the claim and the product beside it is the evidence.

            There was a published setting choosing between the two for a while
            (`homeHeroCombined` on `StoreSettings`). It has been taken out along
            with the stacked arrangement itself — a switch is only worth its
            keep while both of its positions are worth maintaining, and every
            section below had grown a "which layout is this" clause to serve one
            nobody was publishing.

            Not wrapped in `Reveal`: this is above the fold, and the copy
            ...
            wrapping both in a grid, because the two columns have to be siblings
            for the product to sit *beside* the headline rather than under it,
            and the track that scrolls is a client component. Handing the copy
            in keeps it server-rendered. */}
        <FeaturedShowcase
          products={showcaseProducts}
          lead={<HeroCopy categoryNames={categoryNames} showRegister={!nav.user} />}
        />
        ...
        {/*
          ...
          This sat directly beneath the hero, which put the top of the row about
          945px down the page. That is at or above the fold
          ...
          One shelf now instead of the two copies the page carried while it had
          two arrangements to serve: one stated its width in absolute terms
          (`max-w-[calc(80rem-3rem)]`) to line up with a gutterless hero while
          the other used a plain guttered section. Two spellings of one shelf is
          how they drift apart. As its own section it takes the same `max-w-7xl`
          gutters every other shelf here uses.
